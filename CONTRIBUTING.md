...
1. all Issues are automatically given the label `waiting for review`. Please wait until the maintainers review the Issue and update the labels
1. look out for when the Issue label changes to `ready for dev`
...
1. please do not ask to be assigned an issue that has the label `awaiting review` - this creates more notifications for maintainers and it will not be assigned
1. look out for the Issue label `ready for dev` (if it does not have this label, your work might not be accepted)
...
> Notes:
> - any issues that have no activity after 10 days will be unassigned and re-assigned to someone else
